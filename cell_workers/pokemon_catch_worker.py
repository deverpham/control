                    ...
                    if response_dict['responses'][self.response_key][self.response_status_key] is 1:
                        ...
                        if 'wild_pokemon' in response_dict['responses'][self.response_key] or 'pokemon_data' in \
                            ...
                            if 'pokemon_data' in pokemon and 'cp' in pokemon['pokemon_data']:
                                ...
                                msg = ' {pokemon} xuất hiện! [CP {cp}] [Potential {iv}] [S/A/D {iv_display}]'
                                self.emit_event(
                                    'pokemon_appeared',
                                    formatted=msg,
                                    data={
                        ...
                        while True:
                            ...
                            if flag_VIP:
                                ...
                                if catch_rate[pokeball-1] < 0.35 and berries_count > 0 and berry_used == False:
                                    ...
                                    self.emit_event(
                                        'pokemon_catch_rate',
                                        level='debug',
                                        formatted="tỉ lệ bắt {catch_rate} khá thấp. ném {berry_name} ({berry_count} left)",
                                        data={
                                            'catch_rate': success_percentage,
                                            'berry_name': self.item_list[str(berry_id)],
                                            'berry_count': berries_count-1
                                    ...
                                    if response_dict and response_dict['status_code'] is 1 and 'item_capture_mult' in response_dict['responses']['USE_ITEM_CAPTURE']:
                                        ...
                                        self.emit_event(
                                            'threw_berry',
                                            formatted="ném  {berry_name}! tỉ lệ: {new_catch_rate}",
                                            data={
                                                "berry_name": self.item_list[str(berry_id)],
                                                "new_catch_rate": success_percentage
                                            }
                                        ...
                                        if response_dict['status_code'] is 1:
                                            self.emit_event(
                                                'softban',
                                                level='warning',
                                                formatted='không thể dùng bery'
                                            )
                                        else:
                                            self.emit_event(
                                                'threw_berry_failed',
                                                formatted='không thể dùng bery: {status_code}.',
                                                data={
                                                    'status_code': response_dict['status_code']
                                                }
                                            )
                            ...
                            self.emit_event(
                                'threw_pokeball',
                                formatted='Used {pokeball}, tỉ lệ {success_percentage}',
                                data={
                            ...
                            if response_dict and \
                                ...
                                if status is 1:
                                    ...
                                    self.emit_event(
                                        'pokemon_caught',
                                        formatted='Bắt Được {pokemon}! [CP {cp}] [Potential {iv}] [{iv_display}] [+{exp} exp]',
                                        data={
                                            'pokemon': pokemon_name,
                                            'cp': cp,
                                            'iv': pokemon_potential,
                                    ...
                                    if (self.config.evolve_captured
                                        ...
                                        if status == 1:
                                            self.emit_event(
                                                'pokemon_evolved',
                                                formatted="{pokemon} tiến hóa!",
                                                data={'pokemon': pokemon_name}
                                            )
                                        else:
                                            self.emit_event(
                                                'pokemon_evolve_fail',
                                                formatted="tiến hóa thất bại : {pokemon}!",
                                                data={'pokemon': pokemon_name}
                                            )
                            break
        time.sleep(5)
