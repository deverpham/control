...
class SpinFort(BaseTask):
    def should_run(self):
        if not self.bot.has_space_for_loot():
            self.emit_event(
                'inventory_full',
                formatted="Khong du khoang trong"
            )
            return False
        return True
        ...
        if 'responses' in response_dict and \
            ...
            if spin_result == 1:
                ...
                if experience_awarded or items_awarded:
                    self.emit_event(
                        'spun_pokestop',
                        formatted="Nhặt đồ tại pokestop {pokestop}. kinh nghiệm: {exp}. Items: {items}",
                        data={
                            'pokestop': fort_name,
                    ...
                    self.emit_event(
                        'pokestop_empty',
                        formatted='không thấy gì trong pokestop {pokestop}.',
                ...
                self.emit_event(
                    ...
                    formatted="Ra Khỏi Pokestop {pokestop} .",
                    data={'pokestop': fort_name}
                )
            elif spin_result == 3:
                pokestop_cooldown = spin_details.get(
