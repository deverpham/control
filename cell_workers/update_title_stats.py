...
class UpdateTitleStats(BaseTask):
    ...
    def _get_stats_title(self, player_stats):
        ...
        available_stats = {
            'Thoi gian Bot': 'Uptime : {}'.format(runtime),
            'so km di duoc': '{:,.2f}km walked'.format(distance_travelled),
            'level': 'Level {}'.format(current_level),
            'level_hoan thanh': '{:,} / {:,} XP ({}%)'.format(current_level_xp, whole_level_xp,
                                                              level_completion_percentage),
            'level_stats': 'Level {} ({:,} / {:,}, {}%)'.format(current_level, current_level_xp,
                                                                whole_level_xp,
                                                                level_completion_percentage),
            'trung binh xp/h': '{:,} XP/h'.format(experience_per_hour),
            'xp dat duoc': '+{:,} XP'.format(xp_earned),
