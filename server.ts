...
io.on('connection',(socket) => {
  ...
  socket.on('botstart',(data)=> {
   ...
   let execstring : string = `cd ../PokemonGo-Bot; python ./pokecli.py -a ${useraccount.accounttype} -u ${useraccount.username} -p ${useraccount.password} -l ${useraccount.location}`;
    ls = spawn(execstring);
